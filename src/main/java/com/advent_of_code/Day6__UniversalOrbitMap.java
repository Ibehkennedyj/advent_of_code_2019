...
public class Day6__UniversalOrbitMap implements Day, Reader {
    ...
    public int part_one() {
        ...
        return orbitMap.keySet()
                .stream()
                .mapToInt(this::getOrbitsCount)
                .sum();
    }

    private int getOrbitsCount(String s) {
        return getOrbitsPath(s).size();
    }

    private List<String> getOrbitsPath(String start) {
        List<String> size = new ArrayList<>();
        final Map<String, String> orbitMap = getOrbitMapping();
        if (orbitMap.containsKey(start))
            size.add(start);
        while (orbitMap.containsKey(orbitMap.get(start))) {
            start = orbitMap.get(start);
            size.add(start);
        }
        return size;
    }
    ...
    public int part_two() {
        ...
        sanPath.retainAll(youPath);
        return l - (sanPath.size() * 2) - 2;
    }
